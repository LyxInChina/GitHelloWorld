	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		private class PartialInputStream : InflaterInputStream
		{
			...
			public override int ReadByte()
			{
				...
				{
					Stream arg_36_0 = this.baseStream_;
					long offset;
					this.filepos_ = (offset = this.filepos_) + 1L;
					arg_36_0.Seek(offset, SeekOrigin.Begin);
		...
		private int WriteCentralDirectoryHeader(ZipEntry entry)
		{
            ...
            unchecked
            {
                temp = entry.CompressedSize >= (long)((ulong)-1);
            }
            if (entry.IsZip64Forced() || temp)
			{
				this.WriteLEInt(-1);
			}
			else
			{
                unchecked { this.WriteLEInt((int)(entry.CompressedSize & (long)((ulong)-1))); }
			}
            bool temp1;
            unchecked { temp1 = entry.Size >= (long)((ulong)-1); }
			if (entry.IsZip64Forced() || temp1)
			{
				this.WriteLEInt(-1);
			}
			else
			...
			if (entry.CentralHeaderRequiresZip64)
			{
                ...
                if (entry.Size >= _temp || this.useZip64_ == UseZip64.On)
				{
					zipExtraData.AddLeLong(entry.Size);
				}
				if (entry.CompressedSize >= _temp || this.useZip64_ == UseZip64.On)
				{
					zipExtraData.AddLeLong(entry.CompressedSize);
				}
				if (entry.Offset >= _temp)
				{
