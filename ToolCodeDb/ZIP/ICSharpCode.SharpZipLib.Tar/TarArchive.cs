...
namespace ICSharpCode.SharpZipLib.Tar
{
	public class TarArchive : IDisposable
	{
        ...
        private event TestMessageHandler _TestMessageEvent;
        public event TestMessageHandler TestMessageEvent
        {
            [MethodImpl(MethodImplOptions.Synchronized)]
            add { _TestMessageEvent += (TestMessageHandler)Delegate.Combine(_TestMessageEvent, value); }

            [MethodImpl(MethodImplOptions.Synchronized)]
            remove { _TestMessageEvent -= (TestMessageHandler)Delegate.Remove(_TestMessageEvent, value); }
        }

		private bool isDisposed;

        private event ProgressMessageHandler _ProgressMessageEvent;
        ...
        public event ProgressMessageHandler ProgressMessageEvent
		{
			...
			add
			{
				_ProgressMessageEvent += (ProgressMessageHandler)Delegate.Combine(_ProgressMessageEvent, value);
			}
			[MethodImpl(MethodImplOptions.Synchronized)]
			remove
			{
				_ProgressMessageEvent -= (ProgressMessageHandler)Delegate.Remove(_ProgressMessageEvent, value);
			}
		}
		...
		{
            _ProgressMessageEvent?.Invoke(this, entry, message);

			if (_ProgressMessageEvent != null)
			{
                _ProgressMessageEvent(this, entry, message);
			}
		}
